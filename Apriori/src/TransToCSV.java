...
public class TransToCSV {
    ...
    public void run() throws IOException {
        ...
        try {
            BufferedReader fichierTrans = new BufferedReader(new FileReader(m_nomFichier));
            File result = new File("Apriori\\src\\result.csv");
            if (!result.exists()) {
                result.createNewFile();
            }
            BufferedWriter fichierCSV = new BufferedWriter(new FileWriter(result));
            BufferedReader fichierCorrespondance = new BufferedReader(new FileReader("Apriori\\src\\correspondance.txt"));
